...
- **Managed & scheduled launchd service** (building on the on-demand remote-control agent, §6.10):
  - **Supervised agent via `brew services`**: a Homebrew `service` block so `brew services start hark` installs a **per-user LaunchAgent** (login/GUI session) that auto-starts at login, keeping the agent available to the browser userscript (US11) without an open terminal. The bound port comes from the `remote-control-port` config key (default `8473`); engine/model/`directory` come from `hark config`. The service runs with `--no-keep-awake` and **launchd `KeepAlive`** for crash resilience (relaunches are throttled by launchd and visible in the service log). Validated live under launchd: mic capture (TCC prompts attribute directly to the signed `hark`), system/app capture (after a manual "System Audio Recording"/"Screen Recording" grant to the hark binary — see docs/permissions.md), transcription, and the mute API. **macOS 26 caveat:** launchd loads but does not spawn newly-bootstrapped user agents mid-session (`RunAtLoad`/`KeepAlive` only take effect from the next login), so the first `brew services start` needs a one-time `launchctl kickstart`, surfaced in the formula caveats. (A per-user agent stops at logout.)
...
- **Running as a managed service:** the agent is a foreground, SIGTERM-clean process (it blocks until stopped, shuts down cleanly on SIGINT/SIGTERM, and exits 0), so launchd can supervise it via `brew services` — `brew services start hark` runs it as a **per-user LaunchAgent** that auto-starts at login. The bound port is the `remote-control-port` config key (default 8473); set capture/engine defaults with `hark config`. The service runs with `--no-keep-awake` and launchd **`KeepAlive`** for crash resilience (relaunches are throttled and visible in the service log). macOS 26: launchd does not spawn a newly-bootstrapped agent mid-session — the first `brew services start` needs one `launchctl kickstart` (formula caveats). See §4.2, docs/remote-control.md, and docs/permissions.md for the TCC grants the service needs. (Per-user agent; stops at logout — headless/scheduled operation is Post-MVP.)
...
4. ~~**TCC for unbundled CLI:** Confirm the exact permission-attribution behaviour for a signed standalone binary vs. terminal-attributed permission, and document the recommended setup.~~ → **Resolved (validated live on macOS 26, launchd LaunchAgent):** run from a terminal, permissions attribute to the **terminal**; run under **launchd** (brew services), they attribute **directly to the signed `hark` binary** — the microphone prompt appears naming hark and works; "System Audio Recording"/"Screen Recording" never prompt for a CLI and must be **granted manually** to the hark binary in System Settings (both capture backends then work under the service). Caveat: the pane records CLI grants **by path** (the versioned Cellar path), so a Homebrew upgrade may require re-granting — documented in docs/permissions.md. A future app-bundle wrapper would give a stable TCC identity (§4.2).
